...
from core.config import is_ltx_enabled, get_ltx_comfy_url
from core.ltx_comfy import LtxComfyClient, LtxComfyError, build_ltx_workflow
...
# ═══════════════════════════════════════════════════
# Moteur LTX-2.3 (pod GPU loué) — v9.0
# ═══════════════════════════════════════════════════

# Résolutions supportées par le workflow LTX (latent /2 arrondi à ×32)
LTX_DURATION_MAX = 10  # LTX-2.3 (dev) : 25 fps, durée max 10 s par clip
_LTX_WEIGHT = 1  # n'utilise AUCUN crédit Agnes → slot léger


@app.post("/api/tasks/simple-ltx")
async def create_simple_ltx_task(
    request: Request,
    prompt: str = Form(...),
    user_id: str = Header(default="", alias="X-User-Id"),
    duration: int = Form(5),
    ...
    """Crée une tâche vidéo via le moteur LTX-2.3 (pod GPU loué à l'heure).

    Ne consomme AUCUN crédit Agnes : le workflow API est envoyé au pod ComfyUI
    distant (LTX_COMFY_URL), la vidéo MP4 (image + son) est téléchargée puis
    exposée comme une tâche simple classique (GET /api/video/{id}).
    """
    if not is_ltx_enabled():
        raise HTTPException(
            status_code=400,
            detail="Moteur LTX non configuré — définis l'URL du pod ComfyUI "
                   "(LTX_COMFY_URL, ex. https://xxx-8188.proxy.runpod.net) puis redémarre le serveur.",
        )
    ...
    if duration > LTX_DURATION_MAX:
        raise HTTPException(
            status_code=422,
            detail=f"LTX-2.3 (dev) génère au maximum {LTX_DURATION_MAX} s par clip — choisis 5, 7 ou 10 s.",
        )
    ...
    state = SimpleVideoTask(
        task_id=task_id,
        user_id=user_id,
        creative_name=f"simple_ltx_{task_id}",
        prompt=prompt,
        mode=VideoMode.T2V,
        duration=duration,
        video_width=video_width,
    ...
    tm = TaskManager(task_id, dir_name=dir_name)
    tm.create(state)
    _launch_background_task(_run_ltx_with_concurrency(state, tm, dir_name))
    logger.info(f"[LTX] Task created: {task_id}, duration={duration}s (queued)")
    return {"ok": True, "task_id": task_id, "dir_name": dir_name}


async def _run_ltx_with_concurrency(state: SimpleVideoTask, task_manager: TaskManager, dir_name: str):
    """File d'attente légère (semaphore partagé) pour les générations LTX."""
    task_id = state.task_id
    _queued_tasks[task_id] = _LTX_WEIGHT
    logger.info(
        f"[Concurrency] Task {task_id} queued (LTX, weight={_LTX_WEIGHT}, "
        f"current={_pipeline_semaphore.current}/{_pipeline_semaphore.max_weight})"
    )
    task_manager.update_state(status=StepStatus.QUEUED)
    task_manager.update_state(
        current_step="ltx_pod", current_status="running",
        current_message="En attente d'un slot de génération...", current_progress=0.0,
    )
    try:
        await _pipeline_semaphore.acquire(_LTX_WEIGHT)
        _queued_tasks.pop(task_id, None)
        logger.info(f"[Concurrency] Task {task_id} acquired slot (LTX)")
        await _run_ltx(state, task_manager, dir_name)
    except asyncio.CancelledError:
        _queued_tasks.pop(task_id, None)
        logger.info(f"[Concurrency] Task {task_id} cancelled while queued (LTX)")
    finally:
        try:
            await _pipeline_semaphore.release(_LTX_WEIGHT)
        except Exception:
            pass
        _queued_tasks.pop(task_id, None)


async def _run_ltx(state: SimpleVideoTask, task_manager: TaskManager, dir_name: str):
    """Génère la vidéo LTX-2.3 sur le pod : soumission → polling → téléchargement."""
    task_id = state.task_id
    try:
        task_manager.update_state(
            current_step="ltx_pod", current_status="running",
            current_message="Connexion au pod GPU (LTX-2.3)…", current_progress=0.05,
        )
        client = LtxComfyClient(get_ltx_comfy_url())
        ...
        dest_path = os.path.join(dest_dir, "ltx_final.mp4")
        ...
        if not result or not os.path.exists(result):
            raise LtxComfyError("La vidéo n'a pas pu être récupérée depuis le pod.")
        ...
        task_manager.update_state(
            ...
            current_message="Vidéo LTX-2.3 générée avec succès ✓", current_progress=1.0,
        )
        logger.info(f"[LTX] Task {task_id} completed: {result}")

        # v8.1: sauvegarde Supabase (survit au redéploiement Render).
        try:
            await asyncio.to_thread(_persist_video_backup, task_id, result)
        except Exception as e:
            logger.warning(f"[LTX] Sauvegarde vidéo {task_id} impossible: {e}")
    except LtxComfyError as e:
        logger.error(f"[LTX] Task {task_id} failed: {e}")
        task_manager.update_state(
            status=StepStatus.FAILED,
            current_step="ltx_pod", current_status="failed",
            current_message=str(e), current_progress=0.0,
        )
    except Exception as e:
        logger.error(f"[LTX] Task {task_id} failed (inattendu): {e}", exc_info=True)
        task_manager.update_state(
            status=StepStatus.FAILED,
            current_step="ltx_pod", current_status="failed",
